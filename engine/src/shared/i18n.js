...
const M = {
    ...
    'retryOnStatus.retryName': { ru: function(attempt, maxRetries, code) { return '⚡ Повтор ' + attempt + '/' + maxRetries + ' (статус ' + code + ')'; }, en: function(attempt, maxRetries, code) { return '⚡ Retry ' + attempt + '/' + maxRetries + ' (status ' + code + ')'; } },
    'retryOnStatus.retryAfterName': { ru: function(code) { return '⚡ Retry-After: пауза превышает предел (статус ' + code + ')'; }, en: function(code) { return '⚡ Retry-After: wait exceeds cap (status ' + code + ')'; } },
    'retryOnStatus.retryAfterExceeds': { ru: function(waitS, capS) { return 'Сервер запросил повтор через ' + waitS + ' с (заголовок Retry-After), это больше предела ' + capS + ' с — повторы остановлены.'; }, en: function(waitS, capS) { return 'Server asked to retry after ' + waitS + 's (Retry-After header), exceeding the ' + capS + 's cap — retries stopped.'; } },
    'retryOnStatus.retryAfterWait': { ru: function(waitS, requestName) { return '[HEPHAESTUS] ⚡ Retry-After: жду ' + waitS + ' с перед повтором: ' + requestName; }, en: function(waitS, requestName) { return '[HEPHAESTUS] ⚡ Retry-After: waiting ' + waitS + 's before retrying: ' + requestName; } },
    // ─── assertions ───
    ...
    'assertions.maxBytesName':   { ru: function(max, size) { return '📦 Размер ответа ≤ ' + max + ' Б: ' + size + ' Б ' + (size <= max ? '✅' : '❌'); },
                                   en: function(max, size) { return '📦 Response size ≤ ' + max + ' B: ' + size + ' B ' + (size <= max ? '✅' : '❌'); } },
    'assertions.maxBytesExceed': { ru: function(size, max) { return '🚫 Размер ответа превышен: ' + size + ' Б > ' + max + ' Б'; },
                                   en: function(size, max) { return '🚫 Response size exceeded: ' + size + ' B > ' + max + ' B'; } },
