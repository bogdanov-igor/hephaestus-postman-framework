...
- **`checksums.json`** — SHA-256 бандлов; в `engine-update` даёт tamper-evidence при загрузке (ловит битую/обрезанную закачку), но **не** аутентичность: манифест лежит в том же репо и тянется тем же каналом, что и код. Аутентичность — за GitHub-аккаунтом + branch protection; для криптопроверки релизы стоит публиковать signed-тегами (`git tag -s` / `git verify-tag`).
